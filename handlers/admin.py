...
async def addtovars_2(call: types.CallbackQuery, state: FSMContext):
	...
	await call.message.delete()
	await call.message.answer('Отправьте логи:', reply_markup=menu)
...
async def addtovars_(message: types.Message, state: FSMContext):
	...
	file = f'./logs/{message.document.file_id}'
	file_info = await bot.get_file(message.document.file_id)
	name = message.document.file_name.split('.zip')[0]
	namerazdel = data['namerazdel']
	await message.document.download(destination_file=f'./logs/{namerazdel}/' + f"{name}" + ".zip")
	tovarInfo = Razdels.select().where(Razdels.name == namerazdel)[0]
	Tovars.create(name=name, price=tovarInfo.price, razdel=namerazdel)
	date = message.date
	if [message.chat.id, date] not in m:
		m.append([message.chat.id, date])
		await message.answer('Скачиваю товар..')
		await message.answer(f'Товар загружен в раздел <code>{namerazdel}</code>', reply_markup=menuAdmin, parse_mode='html')
...
async def razdel_delete(call: types.CallbackQuery, state: FSMContext):
	...
	os.rmdir('./logs/' + data['dels_razdel'])
